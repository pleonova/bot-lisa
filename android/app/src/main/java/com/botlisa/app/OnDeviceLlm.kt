 ...
 *
 * NOT wired into MainActivity yet -- that's Phase 7. `modelFilePath` is a
 * hardcoded app-files-dir path for now; Phase 5's OnDeviceLlmConfig will own
 * that (and the feature-enabled/WhatElseSource toggle) once it exists --
 * this object will read from it then instead of computing the path locally.
 */
object OnDeviceLlm {
    ...
    fun availability(context: Context): Availability {
        ...
        if (!File(modelFilePath(context)).exists()) return Availability.NOT_DOWNLOADED
    ...
    suspend fun generateWhatElse(context: Context, heard: String): List<Phrase> = mutex.withLock {
        ...
        try {
            ...
            if (!modelLoaded) {
                eng.loadModel(modelFilePath(context))
